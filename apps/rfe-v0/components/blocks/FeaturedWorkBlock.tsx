        ...
        <div className="border-l-2 pl-5 mb-10" style={{ borderColor: 'var(--rfe-gold)', opacity: infoVisible ? 0.8 : 0, transition: 'opacity 2s var(--ease-quiet) 0.5s' }}>
          ...
          {props.attribution && (
            props.externalUrl ? (
              ...
              <span className="text-[10px] tracking-[0.25em] uppercase" style={{ color: 'var(--rfe-gold-dim)' }}>{props.attribution}</span>
            )
          )}
        </div>
      )}
  ...
  return (
    <section data-ai-element="featured-work" className={`relative px-6 lg:px-16 xl:px-24 py-24 lg:py-36 overflow-hidden ${toneClass} section-bleed-top section-bleed-bottom`}>
      ...
      <div className="relative max-w-6xl mx-auto" style={{ zIndex: 2 }}>
        ...
        {isOverlay && posterUrl ? (
          <div ref={cardRef} className="relative overflow-hidden" style={{ aspectRatio: '16/9', background: '#090809', opacity: cardVisible ? 1 : 0, transition: 'opacity 2.5s var(--ease-quiet)' }}>
            <Image src={posterUrl} alt={title} fill className="object-cover" sizes="100vw" style={{ filter: cardVisible ? 'grayscale(0.2) brightness(0.6)' : 'grayscale(1) brightness(0.3)', transition: 'filter 3s var(--ease-quiet)' }} />
            <div className="absolute inset-0 pointer-events-none" style={{ background: 'linear-gradient(to top, rgba(7,7,8,0.92) 0%, rgba(7,7,8,0.4) 50%, transparent 100%)' }} aria-hidden="true" />
